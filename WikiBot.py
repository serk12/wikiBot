...
commands = {'help' : "ask for help",
           ...
           'language' : "set the language of the search",
...
def language(mes):
    get = del_comand('language', mes)
    if (iso_639_1.find(get) != -1):
        id_lang[mes.chat.id] = get
        bot.send_message(mes.chat.id, 'language set to: ' + get)
    else:
        bot.send_message(mes.chat.id, "this language don't exist in ISO 639-1")
